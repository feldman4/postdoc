...
def select_barcodes2(X, min_ions, seed, filter=None):
    ...
    A `filter` function can be provided to remove candidate barcodes based the selected
    barcodes. The input is a binary mask of selected barcodes and the output must be a 
    binary mask that is false only for excluded barcodes.
    """
...
def snake_select_barcodes(df_peptides, METADATA, filter=None):
    ...
    If `require_unique_precursor` is True, 
    """
